...
def generate_monthly_stats_chart():
    ...
    try:
        ...
        for year, month in months:
            ...
            points_redeemed = next((abs(p.redeemed) for p in points_stats if p.year == year and p.month == month), 0)
            points_returned = next((abs(p.returned) for p in points_stats if p.year == year and p.month == month), 0)
            points_earned_data.append(int(points_earned or 0))
            points_redeemed_data.append(int(points_redeemed or 0))
            points_returned_data.append(int(points_returned or 0))
